...
- `explorer` and `reviewer` use free `opencode/big-pickle` and are read-only.
...
- Sleeve is optional and intentionally unused. Do not add
  `SLEEVE_GATEWAY_URL` unless an `anthropic/...` model must be routed through
  that gateway.
